...
import { Music2, Play, PlayCircle, Youtube } from "lucide-react";
...
export function PlayerDock({ selection, active }: PlayerDockProps) {
  ...
  const embedUrl = `https://www.youtube.com/embed/${youtubeVideoId}?start=${selection.startSeconds}&autoplay=1&rel=0`;
  ...
  return (
    <section className="absolute inset-x-4 bottom-4 z-50 overflow-hidden rounded border border-white/12 bg-[#06080d]/94 shadow-2xl backdrop-blur-2xl sm:inset-x-6 lg:left-auto lg:right-8 lg:w-[560px]">
      <div className="grid min-h-[154px] grid-cols-[132px_1fr] gap-0 sm:grid-cols-[230px_1fr]">
        {active ? (
          <div className="relative min-h-[154px] bg-black">
            <iframe
              key={selection.id}
              src={embedUrl}
              title={`${selection.title} — ${selection.timestamp}`}
        ...
        <div className="flex min-w-0 flex-col justify-between gap-3 p-3 sm:gap-4 sm:p-4">
          ...
          <div className="flex flex-wrap gap-2">
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex w-fit items-center gap-2 rounded-full border border-cyanGlow/40 bg-cyanGlow/12 px-3 py-2 text-xs text-white transition hover:border-cyanGlow/70 hover:bg-cyanGlow/18 hover:text-cyanGlow"
            >
              <Youtube size={14} />
              YouTube
            </a>
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex w-fit items-center gap-2 rounded-full border border-coralGlow/40 bg-coralGlow/12 px-3 py-2 text-xs text-white transition hover:border-coralGlow/70 hover:bg-coralGlow/18 hover:text-coralGlow"
            >
              <Music2 size={14} />
              YouTube Music
            </a>
