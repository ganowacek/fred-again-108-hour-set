...
import { Headphones, MapPin, Moon, Search, Sparkles, Sun } from "lucide-react";
...
export default function ArchiveMap() {
  ...
  return (
    ...
    >
      {isLight ? (
        <>
          <div className="pointer-events-none absolute inset-0 z-20 bg-[linear-gradient(90deg,rgba(233,237,243,0.82),rgba(233,237,243,0.05)_38%,rgba(233,237,243,0.05)_70%,rgba(233,237,243,0.55))]" />
          <div className="pointer-events-none absolute inset-0 z-20 bg-[linear-gradient(180deg,rgba(233,237,243,0.7),rgba(233,237,243,0)_30%,rgba(233,237,243,0.2)_84%,rgba(233,237,243,0.62))]" />
        </>
      ) : (
        <>
      ...
      <section className="pointer-events-none absolute left-0 top-0 z-40 flex min-h-[42vh] w-full flex-col justify-between px-5 pb-5 pt-6 sm:px-8 lg:min-h-screen lg:w-[440px] lg:px-10 lg:py-9">
        <div className="pointer-events-auto max-w-[560px]">
          <div className="flex items-center gap-3">
            <span className={`grid size-10 place-items-center rounded-full border shadow-glow backdrop-blur ${iconBadgeClass}`}>
              <Headphones size={18} />
            </span>
          ...
          <label
            className={`mt-5 flex h-12 max-w-[380px] items-center gap-3 rounded-full border px-4 shadow-glow backdrop-blur-xl ${
              isLight ? "border-slate-900/10 bg-white/80" : "border-white/12 bg-black/34"
            }`}
          >
        ...
        <div className={`pointer-events-auto hidden max-w-[360px] rounded border p-4 text-sm backdrop-blur-xl lg:block ${mappedCardClass}`}>
          <p className={`font-mono text-[10px] uppercase tracking-[0.28em] ${isLight ? "text-slate-400" : "text-white/40"}`}>Now mapped</p>
          <p className={`mt-2 text-base ${isLight ? "text-slate-900" : "text-white"}`}>{selectedCity?.city ?? "Choose a city"}</p>
      ...
      {selectedCity ? (
        <CityDrawer
          city={selectedCity}
          currentPlaybackId={isPlayerActive ? playerSelection.id : null}
          onPlayFullShow={(show) => {
            setPlayerSelection(createShowSelection(selectedCity, show));
            setIsPlayerActive(true);
          }}
          onPlaySegment={(show, set) => {
            setPlayerSelection(createSegmentSelection(selectedCity, show, set));
            setIsPlayerActive(true);
          }}
        />
      ) : null}
